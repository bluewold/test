...
class TagMovieHelper(ISqlHelper):
    params= {"tagid":None,"movieid":None}
    _instance_lock = threading.Lock()

    def __new__(cls, *args, **kwargs):
        if not hasattr(TagMovieHelper,"_instance"):
            with TagMovieHelper._instance_lock:
                if not hasattr(TagMovieHelper,"_instance"):
                    TagMovieHelper._instance = object.__new__(cls)
        return TagMovieHelper._instance
    ...
    def insert(self,value=None):
        try:
            tagmovie = TagMovie(tagid=value['tagid'],movieid=value['movieid'])
            self.session.add(tagmovie)
            self.session.commit()
    ...
    def delete(self, conditions=None):
        try:
            if conditions:
                ...
                query = self.session.query(TagMovie)
    ...
    def select(self, count=None, conditions=None):
        ...
        try:
            ...
            query = self.session.query(TagMovie.id,TagMovie.tagid,TagMovie.movieid)
            if len(conditions) > 0 and count:
                for condition in conditions:
                    query = query.filter(condition)
                return query.order_by(TagMovie.movieid.desc()).limit(count).all()
            elif count:
                return query.order_by(TagMovie.movieid.desc()).limit(count).all()
            elif len(conditions) > 0:
                for condition in conditions:
                    query = query.filter(condition)
                return query.order_by(TagMovie.movieid.desc()).all()
            else:
                return query.order_by(TagMovie.movieid.desc()).all()
        except Exception as e:
            print(e)
...
if __name__ == '__main__':
    sqlhelper = TagMovieHelper()
    sqlhelper.init_db()
    actor = {"tagid":3,"movieid":1}
