...
from until.exception import Test_URL_Fail
...
def detect_proxy(proxy, queue2=None):
    ...
    protocol, types, speed = getattr(sys.modules[__name__],config.CHECK_PROXY['function'])( proxies)#checkProxy(selfip, proxies)
    if protocol >= 0:
...
def _checkHttpProxy( proxies, isHttp=True):
    ...
    try:
        start = time.time()
        r = requests.get(url=test_url, headers=HelpTool().get_header(), timeout=config.TIMEOUT, proxies=proxies)
        if r.ok:
...
def getMyIP():
    try:
        r = requests.get(url=config.TEST_IP, headers=HelpTool().get_header(), timeout=config.TIMEOUT)
